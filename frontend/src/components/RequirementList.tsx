...
const RequirementList: React.FC<RequirementListProps> = ({
  ...
  const getStatusIcon = (status: Requirement["status"]) => {
    ...
    return (
      <CloseIcon
        sx={{ color: status === "required" ? "error.main" : "warning.main" }}
      />
    );
  };
  ...
  const formatDetailForDisplay = (
    ...
    if (
      ...
      (normalizedType === "registration" &&
        (detailData.issuer === "DEA" || detailData.type?.includes("DEA")))
    ) {
      const deaDetail = {
          ...
          >
            ...
            >
              {requirement.details?.map(
                (detail: ValidationDetail, detailIndex: number) => (
                  <Box
                    component="div"
                  ...
                  >
                    <RequirementDetail
                      detail={formatDetailForDisplay(
                        detail,
                        requirement.requirement_type || requirement.type || ""
                      )}
                      requirementType={
                        requirement.requirement_type || requirement.type || ""
                      }
