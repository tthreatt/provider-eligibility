...
describe("NPISearch Component", () => {
  ...
  it("handles provider with board certification correctly", async () => {
    ...
    await waitFor(() => {
      // Provider details
      expect(
        screen.getByRole("heading", { name: /Provider: William Martinez/i })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("heading", { name: /NPI: 1104025329/i })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("heading", {
          name: /Provider Type: Allopathic & Osteopathic Physicians/i,
        })
      ).toBeInTheDocument();

      // Board certification details
      const issuerField = screen.getByTestId(
        "detail-field-issuer-abms-american-board-of-medical-specialties"
      );
      expect(issuerField).toHaveTextContent(
        "ABMS - American Board of Medical Specialties"
      );

      const typeField = screen.getByTestId(
        "detail-field-type-internal-medicine-specialty-certificate"
      );
      expect(typeField).toHaveTextContent(
        "Internal Medicine Specialty Certificate"
      );
      ...
      const licenseIssuerField = screen.getByTestId(
        "detail-field-issuer-tennessee"
      );
      expect(licenseIssuerField).toHaveTextContent("Tennessee");

      const licenseNumberField = screen.getByTestId(
        "detail-field-number-50393"
      );
      expect(licenseNumberField).toHaveTextContent("50393");

      // Board Actions
      expect(screen.getByTestId("board-action-1-text")).toHaveTextContent(
        "Warning issued on 2022-01-15"
      );

      // Dates
      expect(
        screen.getByTestId("detail-field-expiration-date-2026-04-01")
      ).toHaveTextContent("2026-04-01");
      expect(
        screen.getByTestId("detail-field-expiration-date-2026-10-31")
  ...
  it("handles provider with CPR certification correctly", async () => {
    ...
    await waitFor(() => {
      // Provider details
      expect(
        screen.getByRole("heading", { name: /Provider: Jane Smith/i })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("heading", { name: /NPI: 1234567890/i })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("heading", {
          name: /Provider Type: Emergency Medical Service Providers/i,
        })
      ).toBeInTheDocument();

      // CPR certification details
      const issuerField = screen.getByTestId(
        "detail-field-issuer-american-heart-association"
      );
      expect(issuerField).toHaveTextContent("American Heart Association");

      const typeField = screen.getByTestId(
        "detail-field-type-cpr-certification"
      );
      ...
      const expDateField = screen.getByTestId(
        "detail-field-expiration-date-2025-12-31"
      );
