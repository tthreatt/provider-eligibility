...
class ValidationRule(Base):
    ...
    id = Column(
        Integer, Sequence("validation_rule_id_seq"), primary_key=True, index=True
    )
...
class BaseRequirement(Base):
    ...
    id = Column(
        Integer, Sequence("base_requirement_id_seq"), primary_key=True, index=True
    )
    ...
    provider_requirements = relationship(
        "ProviderRequirement", back_populates="base_requirement"
    )
...
class ProviderRequirement(Base):
    ...
    id = Column(
        Integer, Sequence("provider_requirement_id_seq"), primary_key=True, index=True
    )
    ...
    override_validation_rules = Column(
        String
    )  # Changed to String to match ValidationRule

    provider_type = relationship("ProviderType", back_populates="requirements")
    base_requirement = relationship(
        "BaseRequirement", back_populates="provider_requirements"
    )
    ...
    def get_validation_rules(self):
        return (
            json.loads(self.override_validation_rules)
            if self.override_validation_rules
            else None
        )
...
DEFAULT_VALIDATION_RULES = {
    ...
    "state_license": {"must_be_active": True, "must_be_unrestricted": True},
    ...
    "work_history": {"must_be_verified": True, "verification_period_years": 5},
}
