...
async def get_auth_token(client: AsyncClient) -> str:
    ...
    auth_response = await client.post(auth_url, headers=headers, json=payload)
...
async def test_search_profile():
    async with AsyncClient(base_url=BASE_URL) as client:
        ...
        response = await client.post(
            ENDPOINT_URL, headers=api_headers, json={"npi": test_npi}
        )
...
async def test_search_sanctions():
    async with AsyncClient(base_url=BASE_URL) as client:
        ...
        response = await client.post(
            SANCTIONS_ENDPOINT_URL, headers=api_headers, json={"npis": test_npis}
        )
